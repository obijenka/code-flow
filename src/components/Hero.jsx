...
export default function Hero() {
  const [mausePosition, setMousePosition] = useState({ x: 0, y: 0 });
  ...
  const currentFloaningCard = floatingCards[activeTab];
  ...
  return (
    <section className="relative min-h-screen flex items-center justify-center pt-16 sm:pt-20 px-4 sm:px-8 overflow-hidden">
      <div
        ...
        style={{
          background: `radial-gradient(600px circle at ${mausePosition.x}px ${mausePosition.y}px, rgba(59, 130, 246, 0.15), transparent 40%)`,
        }}
      ></div>

      <div className="absolute top-20 left-4 sm:left-10 w-48 h-48 sm:h-72 bg-blue-500/10 rounded blur-3xl animate-pulse"></div>
      <div className="absolute bottom-20 right-4 sm:right-10 w-64 sm:h-96 h-64 bg-cyan-500/10 rounded blur-3xl animate-pulse animation-delay-1000"></div>

      <div className="max-w-7xl mx-auto text-center relative w-full">
        <div className="max-w-7xl mx-auto flex flex-col lg:grid lg:grid-cols-2 text-center lg:text-left gap-6 sn:gap-8 lg:gap-12 items-center relative">
          <div>
            ...
            <div className="flex flex-col sm:flex-row align-center justify-center lg:justify-start gap-3 sm:gap-4 mb-8 sm:mb-12 animate-in slide-in-from-bottom duration-700 delay-300">
          ...
          <div className="relative order-2 w-full">
            <div className="relative bg-white/5 backdrop-blur-xl rounded-xl sm:rounded-2xl p-3 sm:p-4 shadow-2xl border border-white/10">
              ...
              {/* FLOATING CARDS */}
              <div
                className={`hidden lg:block absolute bottom-4 right-4 transform translate-y-8 w-72 ${currentFloaningCard.bgColor} backdrop-blur-xl rounded-lg p-4 border border-white/20 shadow-2xl`}
              >
                <div className="flex items-center space-x-2 mb-2">
                  <div
                    className={`w-6 h-6 ${currentFloaningCard.iconColor} flex items-center justify-center text-sm font-bold`}
                  >
                    {currentFloaningCard.icon}
                  </div>
                  <span
                    className={`text-sm font-medium ${currentFloaningCard.textColor}`}
                  >
                    {currentFloaningCard.title}
                  </span>
                </div>
                <div
                  className={`text-sm text-left ${currentFloaningCard.contentColor}`}
                >
                  {currentFloaningCard.content}
                </div>
              </div>
            </div>
          </div>
